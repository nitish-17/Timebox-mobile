...
export function useStore() {
  ...
  const selectedDate = useMemo(() => {
    const dateSetting = settingsArray.find((s) => s.key === "selectedDate");
    return (dateSetting?.value as string) || format(new Date(), "yyyy-MM-dd");
  }, [settingsArray]);

  const navigationSignal = useMemo(() => {
    const signal = settingsArray.find((s) => s.key === "navigationSignal");
  ...
  const uiSettings = useMemo(() => {
    const uiSetting = settingsArray.find((s) => s.key === "uiSettings");
    return (uiSetting?.value as UISettings) || DEFAULT_UI_SETTINGS;
  }, [settingsArray]);
  ...
  const updateUISettings = useCallback(
    async (updates: Partial<UISettings>) => {
      ...
      const newValue = { ...current, ...updates };
      await db.settings.put({ key: "uiSettings", value: newValue });
    },
    [uiSettings],
  );
  ...
  const setDate = useCallback(async (date: string) => {
    await db.transaction("rw", db.settings, async () => {
      await db.settings.put({ key: "selectedDate", value: date });
      const currentSignal = await db.settings.get("navigationSignal");
      const nextSignal = ((currentSignal?.value as number) || 0) + 1;
      await db.settings.put({ key: "navigationSignal", value: nextSignal });
    });
  }, []);
